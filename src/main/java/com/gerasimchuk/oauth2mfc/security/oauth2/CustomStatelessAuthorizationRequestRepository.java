...
import com.gerasimchuk.oauth2mfc.security.helper.EncryptionHelper;
...
public class CustomStatelessAuthorizationRequestRepository implements AuthorizationRequestRepository<OAuth2AuthorizationRequest> {
    ...
    @Autowired
    public CustomStatelessAuthorizationRequestRepository(CookieHelper cookieHelper, EncryptionHelper encryptionHelper) {
    ...
    private OAuth2AuthorizationRequest retrieveCookie(HttpServletRequest request) {
        return cookieHelper.retrieve(request.getCookies(), OAuthHelper.OAUTH_COOKIE_NAME)
                .map(this::decrypt)
                .orElse(null);
    }

    private void attachCookie(HttpServletResponse response, OAuth2AuthorizationRequest value) {
        String cookie = cookieHelper.generateCookie(OAuthHelper.OAUTH_COOKIE_NAME, this.encrypt(value), OAUTH_COOKIE_EXPIRY);
        response.setHeader(HttpHeaders.SET_COOKIE, cookie);
    }

    private void removeCookie(HttpServletResponse response) {
        String expiredCookie = cookieHelper.generateExpiredCookie(OAuthHelper.OAUTH_COOKIE_NAME);
